...
public class ExecutorJobHandler extends IJobHandler {
	...
	@Value("${larkmt.executor.jsonpath}")
	private String jsonPath;

	@Value("${larkmt.shell.path}")
	...
	public ReturnT<String> execute(TriggerParam trigger) {
		...
		try {
			String[] cmdarrayFinal = BuildCommand.buildFlinkXExecutorCmd(trigger, tmpFilePath, flinkXShPath);
			String cmdstr = "";
	...
	private String generateTemJsonFile(String jobJson) {
		...
		if (StringUtils.isNotEmpty(FlinkXHomePath)) {
			jsonPath = FlinkXHomePath + FlinkXConstant.DEFAULT_JSON;
		}
