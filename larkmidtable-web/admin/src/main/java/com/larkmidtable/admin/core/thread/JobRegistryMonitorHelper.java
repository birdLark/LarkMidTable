...
//							JobAdminConfig.getAdminConfig().getJobRegistryMapper().removeDead(ids);
					...
					if (!toStop) {
						logger.error(">>>>>>>>>>> web, job registry monitor thread error:{}", e);
					}
				}
				try {
			...
			logger.info(">>>>>>>>>>> web, job registry monitor thread stop");
		});
		registryThread.setDaemon(true);
		registryThread.setName("web, admin JobRegistryMonitorHelper");
