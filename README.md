...
Lark中文名称云雀，云代表大数据，雀代表平凡和自由。

Lark 对企业的数据采集，预处理，存储，实现为业务赋能和数据的管理。其次数据的规范化（数据仓库规划、数据模型构建、指标定义规范等）统一研发数据，实现数据口径、数据模型标准化。第三是建立业务需求驱动的几大数据体系，深度萃取数据价值；第四是集成数据资产管理能力，从数据的运营、应用、管理、分析、可视化五方面统一管理数据资产。
